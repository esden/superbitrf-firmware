...
if __name__ == "__main__":
    ...
    parser.add_option("--product", type="choice", choices=["any", "Superbit USBRF"],
                      action="store", default="Superbit USBRF",
                      help="only upload to device where idProduct contains PRODUCT\n"
                           "choices: (any, Superbit USBRF), default: Superbit USBRF")
    ...
    # list of tuples with possible stm32 (Superbit USBRF) devices
    stm32devs = []
    ...
    for dev in devs:
        ...
        if man in valid_manufacturers:
            ...
            elif options.product == "Superbit USBRF":
                if "Superbit USBRF" in product:
                    stm32devs.append((dfudev, man, product, serial))
    ...
    if len(stm32devs) > 1:
        ...
        for (d, m, p, s) in stm32devs:
            print("Found possible STM32 (Superbit USBRF) device %s: ID %04x:%04x %s - %s - %s" %
